...
{ buttonId: `#storemenu`, buttonText: { displayText: 'STORE MENU' }, type: 1 }
]
...
try {
console.log(color('Dibatalkan', 'red'), 'Pembelian di batalkan oleh', color(sender.split('@')[0]))
return Ryuu.sendMessage(from, `Baik pembelian akan di batalkan segera...`, text, {quoted: mek}) 
} catch(err) {
e = String(err) 
}
}
...
try {
console.log(color('Dilanjutkan', 'green'), 'Pembelian sedang di tunggu oleh', color(sender.split('@')[0]))
return Ryuu.sendMessage(from, `Pembelian akan di teruskan ke owner\nwa.me/${owner}\nChat untuk melanjutkan pembelian!`, text, {quoted: mek}) 
} catch(err) {
e = String(err) 
}
}
